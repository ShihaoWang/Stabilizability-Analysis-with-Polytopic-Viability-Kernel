  ...
  */
  ...
  try {
    ...
    */
    ...
    std::vector<double> ContactAccOffset(3 * ContactActNo);
    int ContactAccIterIndex = 0;
    for (int i = 0; i < ContactActNo; i++)
    {
      Vector3 ContactRef  = ContactPositionsRef[i];
      Vector3 ContactNow  = ContactPositions[i];
      Vector3 ContactVelo = ContactVelocities[i];
      for (int j = 0; j < 3; j++)
      {
        ContactAccOffset[ContactAccIterIndex] = -Kp * (ContactNow[j] - ContactRef[j]) - Kd * ContactVelo[j];
        ContactAccIterIndex++;
      }
    }
    ...
    for (int i = 0; i < 3 * ContactActNo; i++)
    {
      ...
      GRBLinExpr ConAcc_right_i = ContactAccOffset[i];
    ...
    {
      double Jacc_i = JqddotRes[i - DOF] + Jdotqdot[i - DOF] - ContactAccOffset[i-DOF] - eta_[i - DOF];
...
std::vector<double> StabilizingControllerContact(const Robot& SimRobot, const std::vector<Matrix> & _ActJacobians, const std::vector<Vector3>& _ConeAllUnits, const int & _EdgeNo, const int& _DOF, const double& dt, std::vector<Config>& qTrajDes, std::vector<Config> & qdotTrajDes, std::vector<Config> & qTrajAct, std::vector<Config> & qdotTrajAct, std::vector<LinkInfo> & _RobotLinkInfo, std::vector<ContactStatusInfo> & _RobotContactInfo, const std::vector<Vector3> & _ContactPositionsRef, std::vector<Vector3> & _ContactPositions, std::vector<Vector3> & _ContactVelocities, const int & _ContactPointNo, const int & StepIndex)
{
  ...
  ContactPositionsRef = _ContactPositionsRef;
  ContactPositions = _ContactPositions;
  ContactVelocities = _ContactVelocities;
  ...
  switch (StepIndex)
  {
    ...
    default:
    {
      for (int i = 0; i < 6; i++)
      {
        ...
        {
          if(RobotVelocity_i<-VelLimit)
          {
            RobotVelocityCur[i] = -VelLimit;
          }
          else
          {
            RobotVelocityCur[i] = VelLimit;
